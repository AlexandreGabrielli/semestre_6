...
std::array<Vec2i, 2> boite_englobante(const TGAImage &image, Vec3f *t, int n = 3) {
    ...
    if(box[1].x >= image.get_width()) box[1].x = image.get_width()-1;
...
void triangle(Vec3f *t, TGAImage &image, Vec3f *text, float buffer[width*height] , float intensity ) {
        ...
        for (p.x = bbox[0].x; p.x <= bbox[1].x; ++p.x) {
            ...
            if (b.x >= 0 and b.y >= 0 and b.z >= 0 ) {
                ...
                if ( z >  buffer[p.y*width +p.x] ){
                    ...
                    float x = text[0].x * b.x + text[1].x * b.y +text[2].x * b.z;
                    float y = text[0].y * b.x + text[1].y * b.y +text[2].y * b.z;
...
int main(int argc, char **argv) {
    ...
    if (3 == argc) {
        ...
        char* strTextureTga = "../../obj/african_head_diffuse.tga";
        texture->read_tga_file(strTextureTga);
    ...
    for (int i = 0; i < model->nfaces(); i++) {
        ...
        Vec3f world_coords_text[3];
        for (int j = 0; j < 3; j++) {
            world_coords_text[j] = model->texture(face_text[j]);
            world_coords[j] = model->vert(face[j]);
            t[j] = Vec3f((world_coords[j].x + 1.) * width / 2., (world_coords[j].y + 1.) * height / 2.,
                         world_coords[j].z);
            tt[j] = Vec3f((world_coords_text[j].x ) * texture->get_width() , (world_coords_text[j].y ) * texture->get_height(),
                          world_coords_text[j].z);
        }
